...
const PatientModalComponent: React.FC<PatientModalProps> = ({
  ...
  const initialValues = useMemo((): CreatePatient | UpdatePatient => {
    if (mode === 'create') {
      ...
      const initialValues: CreatePatient | UpdatePatient = {
        ...
        ...(mode === 'edit' && patientData.id ? { id: patientData.id } : {}),
      };
  ...
  const handleSubmit = useCallback((values: CreatePatient | UpdatePatient) => {
                ...
                <Field name="age">
                  {({ field }: FieldProps) => (
                    <Input
                      ...
                      type="number"
                      onChange={(e) =>
                        setFieldValue('age', e.target.value ? parseInt(e.target.value) : undefined)
                      }
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <ErrorMessage name="consents" component="p" className="text-red-500 text-xs mt-1" />
              </div>
            </div>
          </div>
  ...
  ), [initialValues, mode, handleSubmit, onClose, isLoading]);
